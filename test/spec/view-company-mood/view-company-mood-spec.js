...
import customCommands from "../../pageObject/commands";
...
When("user click {string} button", async (btnMood) => {
  ...
  await driver.pause(3000);
  await $(
    '(//android.widget.ImageView[@content-desc="' + btnMood + '"])'
  ).click();
});
...
Then("{string} screen should appear", async (text) => {
  await expect(
    $('//android.view.View[@content-desc="' + text + '"]')
  ).toExist();
});
...
Then("{string} should be displayed", async (btn) => {
  await expect(
    $('(//android.widget.Button[@content-desc="￼ ' + btn + '"])').toEqual(2)
  );
});
...
Then("user should be able to unfollow people", async () => {
  ...
  await expect(
    $('(//android.widget.Button[@content-desc="￼ Following"])').toEqual(1)
  );
});
...
Then("the person should appear below", async () => {
  // await expect(
  //   $('//android.view.View[@content-desc="Sadie Sink Backend"]')
  // ).toHaveTextContaining("Sadie Sink");
});
